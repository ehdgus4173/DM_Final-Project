...
**하니스**: 사다리 *개념*은 `Analysis1_title/LR/r2_ceiling_test.py`, **평가 하니스는 A3 `step3_channel_analysis.ipynb`의 `perf()`/`cv_r2()` 미러** (⚠️ 아래 §B 참조)
...
| 9 | CV | `KFold(5, shuffle, rs=42)`, 각 fold도 category 필터 | A3 `cv_r2()` 동일 |
...
> **⚠️ 하니스 주의 (재현 핵심)**: `r2_ceiling_test.py`의 eval 함수는 **재사용 금지**. 그것은 `cleaned_USvideos.csv`를 base로 `publish_hour`/`publish_weekday`를 **수치형**으로 넣고, hour_bin Treatment·category 필터가 없어 **T0=0.119를 재현 못 한다.** A4는 **A3 step3의 `perf()`/`cv_r2()` + formula 방식을 그대로 미러**하고 `FEATURE_SETS`만 T0~T3 4단으로 확장한다. (r2_ceiling_test는 "사후변수 단계 추가" 아이디어의 출처일 뿐.)
...
- **작업**
  - A3 `perf(F, tr, te)` / `cv_r2(F, d)` 함수 복제 (category 필터 포함)
